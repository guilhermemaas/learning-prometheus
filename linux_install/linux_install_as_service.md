...
#Copiar/mover o executável para um diretório de binários mapeado no PATH:
cp prometheus-2.48.1.linux-amd64/usr/local/bin/

#Copiar/mover o promtool
cp prometheus-2.48.1.linux-amd64/promtool /usr/local/bin

#Criar um arquivo de configuração e copiar o config:
mkdir /etc/prometheus && cp prometheus-2.48.1.linux-amd64/prometheus.yml /etc/prometheus/
...
#Criar um service unit pro systemd:
...
#Acertar as permissões das pastas:
...
#Rodar um reload nos daemons do systemctl
sudo systemctl daemon-reload

#Ativar o serviço do Prometheus
systemctl enable prometheus

#Se tiver problemas com serviço
